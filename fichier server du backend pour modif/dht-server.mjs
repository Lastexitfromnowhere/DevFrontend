...
// Configuration CORS - Désactivée pour laisser Nginx gérer les en-têtes CORS
app.use(cors({
  origin: false, // Désactive la gestion des origines par Express
  credentials: true
}));
...
let dhtRouter;
try {
  // Essayer d'abord d'importer la version multi-device
  dhtRouter = (await import('./routes/dht.multi.mjs')).default;
  console.log('✅ Routes DHT multi-device chargées avec succès');
} catch (error) {
  console.warn('⚠️ Impossible de charger dht.multi.mjs, tentative avec dht.mjs');
  
  try {
    // Ensuite essayer la version ESM standard
    dhtRouter = (await import('./routes/dht.mjs')).default;
    console.log('✅ Routes DHT standard (ESM) chargées avec succès');
  } catch (error) {
